...
const MyGoalsScreen = () => {
  ...
  const aggregateActivityForGoal = (goal, categoryActivities) => {
    let aggregated = { steps: 0, duration: 0, sessions: 0, weightLost: 0 };
    const goalStart = new Date(goal.dateCreated);
    goalStart.setHours(0, 0, 0, 0);
    const goalEnd = new Date(goal.deadline);
    goalEnd.setHours(23, 59, 59, 999);
    ...
    Object.values(categoryActivities).forEach((entry) => {
      ...
      if (entryDate >= goalStart && entryDate <= goalEnd) {
        ...
        } else if (goal.category === 'Lose Weight') {
          aggregated.weightLost += Number(entry.weightLost) || 0;
        }
      }
    });
    return aggregated;
  ...
  useEffect(() => {
    ...
    userGoals.forEach((goal) => {
      if (!goal.completed) {
        ...
        if (goal.category === 'Cardio') {
          const targetSteps = Number(goal.steps) || 0;
          const targetDuration = Number(goal.duration) || 0;
          if (aggregated.steps >= targetSteps && aggregated.duration >= targetDuration) meetsTarget = true;
        } else if (goal.category === 'Yoga') {
          const targetDuration = Number(goal.duration) || 0;
          if (aggregated.duration >= targetDuration) meetsTarget = true;
        } else if (goal.category === 'Strength') {
          const targetSessions = Number(goal.sessions) || 0;
          if (aggregated.sessions >= targetSessions) meetsTarget = true;
        } else if (goal.category === 'Lose Weight') {
          const targetLoss = Number(goal.weight) || 0;
          if (aggregated.weightLost >= targetLoss) meetsTarget = true;
        }
  ...
  const handleSubmit = () => {
    if (selectedCategory === 'Select Goal' || goalValue.trim() === '') {
    ...
    if (user) {
      ...
      if (selectedCategory === 'Cardio') {
        const [steps, duration] = goalValue.split(',');
        goalData = { ...goalData, steps: steps.trim(), duration: duration.trim() };
      } else if (selectedCategory === 'Yoga') {
        goalData = { ...goalData, duration: goalValue.trim() };
      } else if (selectedCategory === 'Strength') {
        goalData = { ...goalData, sessions: goalValue.trim() };
      } else if (selectedCategory === 'Lose Weight') {
        goalData = { ...goalData, weight: goalValue.trim() };
      }
      ...
      set(newGoalRef, goalData)
        ...
        .catch((error) => alert('Failed to save goal. Please try again.'));
    }
  };
  ...
  return (
    <View style={styles.container}>
      <ScrollView style={styles.scrollContainer}>
        <TouchableOpacity style={styles.createGoalButton} onPress={() => setShowGoalForm(!showGoalForm)}>
          <Text style={styles.createGoalButtonText}>{showGoalForm ? 'Cancel' : 'Create a New Goal'}</Text>
        </TouchableOpacity>
        ...
        {showGoalForm && (
          <View style={styles.goalContainer}>
            ...
            <Picker selectedValue={selectedCategory} onValueChange={setSelectedCategory} style={styles.picker}>
              <Picker.Item label="Select Goal" value="Select Goal" />
              <Picker.Item label="Cardio (steps, duration)" value="Cardio" />
              <Picker.Item label="Yoga (duration in minutes)" value="Yoga" />
              <Picker.Item label="Strength (number of sessions)" value="Strength" />
              <Picker.Item label="Lose Weight (kg)" value="Lose Weight" />
            </Picker>

            <TextInput
              style={styles.input}
              placeholder={selectedCategory === 'Cardio' ? 'e.g., 10000, 30' : 'Enter value'}
              value={goalValue}
            ...
            <TouchableOpacity onPress={() => setShowDatePicker(true)}>
              <Text style={styles.dateText}>{goalDeadline.toLocaleDateString()}</Text>
            </TouchableOpacity>
            {showDatePicker && (
              <DateTimePicker
                value={goalDeadline}
        ...
        {/* Display existing goals logic remains the same */}
      </ScrollView>
      <NavBar />
    </View>
  );
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  scrollContainer: { flexGrow: 1, paddingBottom: 70 },
  createGoalButton: { backgroundColor: '#09355c', padding: 10, borderRadius: 5, margin: 10 },
  createGoalButtonText: { color: 'white', textAlign: 'center', fontSize: 18 },
  goalContainer: {
    marginTop: 20,
    padding: 10,
  ...
  dateLabel: { fontSize: 16, marginBottom: 5 },
  dateText: { fontSize: 16, color: '#09355c', marginBottom: 10 },
});
