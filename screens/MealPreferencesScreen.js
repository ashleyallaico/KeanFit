...
export default function SeedCaloriesScreen() {
    const userId = auth.currentUser?.uid;
    const db = getDatabase();

    const seedCalories = async () => {
        if (!userId) {
        ...
        const data = {
            calories: 2200, // Example value, you can update this dynamically later
        };

        try {
            await set(caloriesRef, data);
            Alert.alert('Success', 'Calories data has been seeded!');
            console.log(`Calories set for user ${userId}`);
        } catch (error) {
            console.error('Error seeding calories:', error);
            Alert.alert('Error', 'Failed to seed calories data.');
        }
    };

    return (
        <View style={styles.container}>
            <Text style={styles.title}>Seed Calories</Text>
            <TouchableOpacity style={styles.button} onPress={seedCalories}>
                <Text style={styles.buttonText}>Create Calories Table</Text>
            </TouchableOpacity>
        </View>
    );
}
...
const styles = StyleSheet.create({
    ...
    button: { backgroundColor: '#09355c', padding: 15, borderRadius: 10 },
